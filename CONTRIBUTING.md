...
This repo packages [Clams Remote](https://github.com/clams-tech/Remote) for StartOS.
...
```bash
git submodule update --init --recursive    # initialize the clams-remote submodule
npm ci                                     # install dependencies
make                                       # build the universal .s9pk
```

The `clams-remote` image is built from the `clams-remote/` git submodule — it must be checked out before `make`.

## Updating the upstream version

Clams Remote is built from the `clams-remote/` git submodule. There is no `dockerTag` in the manifest.
